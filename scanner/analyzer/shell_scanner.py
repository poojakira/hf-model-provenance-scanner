...
_PATTERNS = [
    ...
    (
        ...
        "HFS-015",
        "Defender exclusion path modification",
    ),
    (
    ...
    (re.compile(r"Start-BitsTransfer", re.IGNORECASE), "HFS-023", "BITS transfer download"),
    ...
    (re.compile(r"schtasks\s+/create", re.IGNORECASE), "HFS-016", "Scheduled task creation"),
    (re.compile(r"Register-ScheduledTask", re.IGNORECASE), "HFS-016", "Register-ScheduledTask"),
    (re.compile(r"New-ScheduledTask", re.IGNORECASE), "HFS-016", "New-ScheduledTask"),
    (
        re.compile(r"HKCU.*\\Run|HKLM.*\\Run", re.IGNORECASE),
        "HFS-016",
        "Registry Run key persistence",
    ),
    (re.compile(r"crontab\s+-|@reboot", re.IGNORECASE), "HFS-016", "cron persistence"),
    # HFS-014: Hidden window execution
    (re.compile(r"WindowStyle\s+Hidden", re.IGNORECASE), "HFS-014", "Hidden WindowStyle"),
    (re.compile(r"CREATE_NO_WINDOW|0x08000000", re.IGNORECASE), "HFS-014", "CREATE_NO_WINDOW flag"),
    (re.compile(r"SW_HIDE", re.IGNORECASE), "HFS-014", "SW_HIDE window flag"),
    (
        re.compile(r"creationflags.*0x08000000", re.IGNORECASE),
        "HFS-014",
        "creationflags hidden window",
    ),
    # HFS-004: Paste services / C2 endpoints
    (
        re.compile(r"pastebin\.com|hastebin\.com|paste\.ee", re.IGNORECASE),
        "HFS-004",
        "Paste service C2 endpoint",
    ),
    (re.compile(r"ngrok\.io|ngrok-free\.app", re.IGNORECASE), "HFS-004", "ngrok tunnel endpoint"),
    (re.compile(r"trycloudflare\.com", re.IGNORECASE), "HFS-004", "Cloudflare tunnel endpoint"),
    (
        re.compile(r"webhook\.site|requestbin\.net", re.IGNORECASE),
        "HFS-004",
        "Webhook/requestbin endpoint",
    ),
    (
        re.compile(r"transfer\.sh|anonfiles\.com", re.IGNORECASE),
        "HFS-004",
        "Anonymous file sharing endpoint",
    ),
]
