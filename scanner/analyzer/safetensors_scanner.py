...
def analyze_safetensors_file(file_path: str, data: bytes) -> list[Finding]:
    ...
    if len(data) < SAFETENSORS_HEADER_SIZE_BYTES:
        findings.append(
            _make_finding(
                "HFS-055", file_path, "File too small to be valid SafeTensors (< 8 bytes)"
            )
        )
        return findings
    ...
    header_bytes = data[SAFETENSORS_HEADER_SIZE_BYTES : SAFETENSORS_HEADER_SIZE_BYTES + header_size]

    try:
        header_json = json.loads(header_bytes.decode("utf-8"))
    except (json.JSONDecodeError, UnicodeDecodeError) as e:
        findings.append(
            _make_finding("HFS-055", file_path, f"SafeTensors header is not valid JSON: {e}")
        )
        return findings
...
def _scan_metadata(file_path: str, metadata: dict, findings: list[Finding]):
    ...
    for key, value in metadata.items():
        ...
        _check_string_for_injection(file_path, f"__metadata__.{key}", value, findings)
...
def _check_string_for_injection(file_path: str, context: str, value: str, findings: list[Finding]):
