...
def analyze_source_file(
    file_path: str, source: str, raw_data: bytes = b""
) -> list[Finding]:
...
def add_remote_policy_findings(
    result: ScanResult, repo_id: str, files: list[str], config: dict
):
...
def scan_local(
    ...
    if os.path.isfile(target) and _is_binary_model(target):
    ...
    for file_path, is_oversized in files_to_scan:
        ...
        try:
            ...
            result.files_skipped += 1
            result.findings.append(
                make_finding("HFS-098", file_path=file_path, evidence=str(exc))
            )
            continue
...
def scan_remote_files(
    ...
    try:
        ...
        reason = {
            401: "gated (requires authentication)",
            403: "private or forbidden",
            404: "not found",
        }.get(exc.code, f"unavailable (HTTP {exc.code})")
        result.findings.append(
            make_finding(
                "HFS-096",
                file_path=repo_id,
    ...
    for filename in files:
        ...
        try:
            ...
            result.files_skipped += 1
            result.findings.append(
                make_finding("HFS-098", file_path=filename, evidence=str(exc))
            )
            continue
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Include INFO findings in output"
    )
    parser.add_argument(
        "-q",
        "--quiet",
    ...
    parser.add_argument(
        "--version", action="version", version=f"hf-scanner {SCANNER_VERSION}"
    )
    # New v0.2 flags
    parser.add_argument(
        "--baseline",
...
def main(argv=None):
    ...
    if is_local_dir and mode == "both":
        ...
        print(
            f"Error: Target '{args.target}' is not a local directory, "
            f"but mode is 'local'",
            file=sys.stderr,
        )
        return 3
    ...
    if not args.quiet:
        if out_format == "json":
            ...
            counts = {
                s: sum(1 for f in result.findings if f.severity == s) for s in Severity
            }
            lines.append(
    ...
    if fail_threshold_str != "never":
        ...
        if any(
            severity_order.get(f.severity.value, 0) >= threshold_val
            for f in result.findings
        ):
            return 1
    return 0
