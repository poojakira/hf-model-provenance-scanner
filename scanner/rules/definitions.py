...
RULES: dict[str, Rule] = {
    ...
    "HFS-041": Rule("HFS-041", "signature-not-trust-anchored", Severity.MEDIUM, "cosign signature present but no trusted key/identity was configured, so verification cannot establish WHO signed it (a bare keyless verify-blob trusts any Sigstore/Rekor entry)", "Configure HFS_COSIGN_KEY, or HFS_COSIGN_CERT_IDENTITY[_REGEXP] + HFS_COSIGN_OIDC_ISSUER, so signatures are checked against a trusted signer.", "CWE-347", ["provenance", "signature"]),
