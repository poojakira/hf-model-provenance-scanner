...
BINARY_DIR = os.path.join(_FIXTURES_DIR, "binary")
...
class GatedClient:
    ...
    def download_file(self, repo_id, filename):
        raise HFAccessError(
            401, f"https://huggingface.co/{repo_id}/resolve/main/{filename}"
        )
...
class TestCli(unittest.TestCase):
    def test_local_fail_on_high_returns_1(self):
        code = cli.main(
            [MALICIOUS_DIR, "--mode", "local", "--quiet", "--fail-on", "high"]
        )
        self.assertEqual(code, 1)

    def test_local_fail_on_never_returns_0(self):
        code = cli.main(
            [MALICIOUS_DIR, "--mode", "local", "--quiet", "--fail-on", "never"]
        )
        self.assertEqual(code, 0)

    def test_direct_binary_target_is_scanned(self):
        target = os.path.join(BINARY_DIR, "malicious_os_system.pkl")
        stdout = io.StringIO()
        with redirect_stdout(stdout):
            code = cli.main(
                [target, "--mode", "local", "--format", "json", "--fail-on", "never"]
            )

        self.assertEqual(code, 0)
        report = json.loads(stdout.getvalue())
        self.assertEqual(report["files_scanned"], 1)
        self.assertIn("HFS-050", {finding["rule_id"] for finding in report["findings"]})
    ...
    def test_approved_publisher_policy_blocks_unknown_vendor(self):
        ...
        try:
            ...
            report = json.loads(stdout.getvalue())
            self.assertIn(
                "HFS-034", {finding["rule_id"] for finding in report["findings"]}
            )
        finally:
    ...
    def test_runtime_policy_output(self):
        with tempfile.NamedTemporaryFile(
            "r", encoding="utf-8", suffix=".json", delete=False
        ) as f:
            policy_path = f.name
        try:
            code = cli.main(
    ...
    def test_html_report_output(self):
        with tempfile.NamedTemporaryFile(
            "r", encoding="utf-8", suffix=".html", delete=False
        ) as f:
            report_path = f.name
        try:
            code = cli.main(
